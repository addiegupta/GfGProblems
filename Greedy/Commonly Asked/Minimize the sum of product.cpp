...
GeeksForGeeks: Minimize the sum of product

Link to problem: https://practice.geeksforgeeks.org/problems/minimize-the-sum-of-product/0
...
// Sort arrays, one in increasing order, one in decreasing order
// Multiply corresponding elements to get minimum sum of products
ll minSumOfProducts(ll a[],ll b[],int n){
	ll ans = 0;
	sort(a,a+n);
...
int main(){
	...
	while(t--){
		...
		cout<<minSumOfProducts(a,b,n)<<endl;
	}
}